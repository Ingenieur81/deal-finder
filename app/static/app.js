...
function resetForm() { $('#item-form').reset(); $('#currency').value='USD'; $('#enabled').checked=true; $('#item-id').value=''; $('#form-title').textContent='Add an item'; $('#cancel').hidden=true; $('#target-label').firstChild.textContent='Email address'; $('#notification_target').type='email'; }
function edit(item) { $('#item-id').value=item.id; ['name','region','min_price','max_price','currency','notification_method','notification_target'].forEach(key => $( '#'+key ).value=item[key] ?? ''); $('#enabled').checked=item.enabled; $('#form-title').textContent=`Edit: ${item.name}`; $('#cancel').hidden=false; window.scrollTo({top:0,behavior:'smooth'}); }
...
$('#notification_method').onchange = () => { const android=$('#notification_method').value==='android'; $('#target-label').firstChild.textContent=android?'FCM device token':'Email address'; $('#notification_target').type=android?'text':'email'; $('#notification_target').placeholder=android?'Firebase registration token':'you@example.com'; };
