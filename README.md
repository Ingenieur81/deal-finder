...
- Search for lowest available price only
- Save the previous found prices so comparing with the current is possible
- Show lowest found price in the GUI next to Target price
- Add a URL to the lowest price to the item name in the overview
- Only send a notification when the current price is lower than the previous
- Location / region should be only countries and should be a dropdown with available countries to search in, default is The Netherlands
- Currency default should be EUR and a dropdown list of available currencies